...
class SECTools:
    ...
    @async_retry(max_retries=3, base_delay=1.0, exceptions=(aiohttp.ClientError, asyncio.TimeoutError, SECToolsError))
    async def _search_filing(self, query: str, form_type: str) -> str:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                filings = await with_semaphore(self.semaphore, queryApi.get_filings_async, query, session=session)
            ...
            raise SECToolsError(f"Failed to complete {form_type} search after multiple attempts. Please try again later.")
        except Exception as e:
            logger.error(f"Error in {form_type} search: {e}")
            raise SECToolsError(f"Error in {form_type} search: {str(e)}")

    @async_retry(max_retries=3, base_delay=1.0, exceptions=(aiohttp.ClientError, asyncio.TimeoutError, EmbeddingSearchError))
