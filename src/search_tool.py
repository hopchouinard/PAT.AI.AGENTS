...
from exceptions import SearchToolError, SearchQueryError
...
class SearchTool:
    ...
    async def create_search_tool(self) -> Tool:
        @async_retry(max_retries=3, base_delay=1.0, exceptions=(aiohttp.ClientError, asyncio.TimeoutError, SearchToolError))
        async def search_function(*args: Any, **kwargs: Any) -> str:
            ...
            try:
                ...
                raise SearchToolError(f"Failed to complete search after multiple attempts. Please try again later.")
            except Exception as e:
                logger.error(f"Error in search function: {e}")
                raise SearchToolError(f"An error occurred while processing the search query: {str(e)}")
