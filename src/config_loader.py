...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    if (
        not isinstance(config.get("embedding_chunk_size"), int)
        or config.get("embedding_chunk_size", 0) <= 0
    ):
        raise InvalidConfigError("embedding_chunk_size must be a positive integer")

    if (
        not isinstance(config.get("embedding_chunk_overlap"), int)
        or config.get("embedding_chunk_overlap", 0) < 0
    ):
        raise InvalidConfigError(
            "embedding_chunk_overlap must be a non-negative integer"
        )
...
def validate_crew_config(config: Dict[str, Any]) -> None:
    ...
    if "agents" not in config or not isinstance(config["agents"], dict):
        raise InvalidConfigError(
            "Crew configuration must contain an 'agents' dictionary"
        )
