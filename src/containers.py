...
class Container(containers.DeclarativeContainer):
    ...
    # Core components
    ollama_llm = providers.Factory(Ollama, model=config.default_llm_model)

    sec_tools = providers.Factory(
        SECTools, config=config, sec_api_key=config.sec_api_key.get_secret_value()
    )
    ...
    task_manager = providers.Factory(TaskManager, config=config)

    crew_runner = providers.Factory(CrewRunner, config=config)
