...
from src.exceptions import SECToolsError
...
async def test_search_10q_complex_query(sec_tools):
    result = await sec_tools.search_10q(
        "GOOGL|What were the main risks mentioned in the last quarter?"
    )
    assert isinstance(result, str)
    assert len(result) > 0
...
async def test_search_10k_complex_query(sec_tools):
    result = await sec_tools.search_10k(
        "AMZN|What were the major acquisitions in the last year?"
    )
    assert isinstance(result, str)
    assert len(result) > 0
...
async def test_search_with_special_characters(sec_tools):
    ...
    assert len(result) > 0
    assert any(
        phrase in result.lower() for phrase in ["r&d", "research and development"]
    )
