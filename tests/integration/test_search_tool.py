...
import pytest
from src.search_tool import SearchTool, create_search_tool
from src.config import config
from src.exceptions import SearchToolError, SearchQueryError
import asyncio
...
async def test_search_complex_query(search_tool):
    ...
    assert any(lang.lower() in result.lower() for lang in ["python", "javascript", "java", "c++", "go"])
...
async def test_search_rate_limiting():
    search_tool = await create_search_tool(config, config.serper_api_key.get_secret_value())
...
async def test_search_result_processing(search_tool):
    ...
    assert isinstance(result, str)
    assert len(result) > 0
    assert len(result) <= config.search_result_limit  # Check if result is within the configured limit
