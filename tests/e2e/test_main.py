...
async def test_main_successful_execution(mock_dependencies):
    ...
    with patch("builtins.input", return_value="1"):
        ...
        with patch(
            "src.main.get_available_crew_configs", return_value=["test_config.yaml"]
        ):
            with patch(
                "src.main.load_crew_config",
                return_value={
                    "agents": {"test_agent": {}},
                ...
                mock_agent_manager.create_agents.return_value = {
...
async def test_main_invalid_crew_choice(mock_dependencies):
    with patch(
        ...
        with patch(
            "src.main.get_available_crew_configs",
            return_value=["config1.yaml", "config2.yaml"],
        ):
...
async def test_main_api_key_error(mock_dependencies):
    with patch(
        "src.main.setup_environment", side_effect=APIKeyError("Missing API key")
    ):
    ...
    with patch("builtins.input", return_value="1"):
        with patch(
            ...
            with patch("src.main.load_crew_config", return_value={}):
                ...
                mock_task_manager.create_tasks.return_value = [MagicMock()]
                mock_crew_runner.run_crew.side_effect = CrewExecutionError(
                    "Execution failed"
                )
...
async def test_main_unexpected_error(mock_dependencies):
    ...
    with patch("builtins.input", return_value="1"):
        with patch(
            ...
            with patch(
                "src.main.load_crew_config", side_effect=Exception("Unexpected error")
            ):
                with pytest.raises(SystemExit):
                    await async_main()
...
async def test_main_with_real_config(mock_dependencies):
    ...
    with patch("builtins.input", return_value="1"):
        with patch(
            "src.main.get_available_crew_configs", return_value=[real_config_path]
        ):
            ...
            with patch("src.main.load_crew_config", return_value=real_config):
                mock_agent_manager.create_agents.return_value = {
                    agent: MagicMock() for agent in real_config["agents"]
                }
                mock_task_manager.create_tasks.return_value = [
                    MagicMock() for _ in real_config["tasks"]
                ]
